...
export class TokenInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    if (!request.url.toLowerCase().startsWith('http') && !request.url.toLowerCase().startsWith('/assets')) {
      request = request.clone({
        url: environment.apiUrl + request.url
      });
    }

    if (!request.url.toLowerCase().startsWith('https://build.') && !request.headers.has('Authorization')) {
        ...
        if (event instanceof HttpResponse) {
          if (event.body && event.body.status && event.body.status.toLowerCase() === 'error') {
            if (event.body.result === 10007) {
              if (!request.url.includes('/agencyForAdmin/')) {
